...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "WITH RECURSIVE years(year) AS (VALUES(2000) UNION ALL SELECT year + 1 FROM years WHERE year < 2024) SELECT year AS 'Год', CASE strftime('%w', year || '-01-02') WHEN '0' THEN 'Воскресенье' WHEN '1' THEN 'Понедельник' WHEN '2' THEN 'Вторник' WHEN '3' THEN 'Среда' WHEN '4' THEN 'Четверг' WHEN '5' THEN 'Пятница' WHEN '6' THEN 'Суббота' END AS 'День недели' FROM years ORDER BY year;"
echo " "